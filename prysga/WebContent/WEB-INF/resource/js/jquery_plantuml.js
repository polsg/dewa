...
var deflater = window.SharedWorker && new SharedWorker('resource/js/rawdeflate.js');
if (deflater) {
  deflater.port.addEventListener('message', done_deflating, false);
  deflater.port.start();
} else if (window.Worker) {
  deflater = new Worker('resource/js/rawdeflate.js');
...
$("img").each(function () {
  ...
  if (u2=="") return;
  $(this).attr("src", "http://localhost:8080/plantuml/img/"+encode64(e.data));
